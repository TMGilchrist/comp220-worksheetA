...
void OpenGLBulletDebugDrawer::CreateShader()
{
	...
	DebugShader->Load("vertexShader.glsl", "fragmentShader.glsl");
}
...
void OpenGLBulletDebugDrawer::drawLine(const btVector3 & from, const btVector3 & to, const btVector3 & color)
{
	...
	//Shouldn't be deleting and recreating these. Should create one large buffer. But not necessary for this debug code.
	glDeleteBuffers(1, &VBO);
	glDeleteVertexArrays(1, &VAO);
